...
export class SellerRepository extends DBOperation {
  ...
  async updateAddress(input: AddressInput & { user_id: number }) {
    ...
    if (addressResult.rowCount > 0) {
      queryString = `UPDATE address SET
        address_line1=$1,
        address_line2=$2,
        city=$3,
        post_code=$4,
  ...
  async getPaymentMethods(userId: number) {
    ...
    console.log("queryString", queryString, userId);
    const result = await this.executeQuery(queryString, [userId]);
    console.log("resultresult", result.rowCount);

    if (result.rowCount > 0) {
      return result.rows[0] as PaymentMethodModel;
    }
    return new Error("Payment Method Does Not Exists");
  }
  ...
  async updatePaymentMethods({
    ...
    const queryString = `UPDATE payment_methods SET bank_account=$1, swift_code=$2, payment_type=$3 WHERE _id=$4 AND user_id=$5`;
    const values = [
